...
public class ExerciseStatusService {
    ...
    public ExerciseStatusDto getExerciseStatusDto(int userId) {
        ExerciseStatusDto dto = new ExerciseStatusDto();

        /** ✅ 1. 이번 달 총 운동 시간 */
        int totalMinutes = exerciseStatusMapper.getMonthlyTotalTime(userId);
        String formattedTime = (totalMinutes / 60) + "시간 " + (totalMinutes % 60) + "분";
        dto.setMonthlyTotalTime(formattedTime);
        ...
        dto.setTotalRoutineCount(totalRoutineCount);
        dto.setCompletedRoutineSessions(completedSessions);
        dto.setConsistencyScore(consistencyScore);


        /** ✅ 3. 평균 운동 완료율 */
        double avgCompletionRate = exerciseStatusMapper.getAverageCompletionRate(userId);
        dto.setAverageComletionRate(Math.round(avgCompletionRate));

        /** ✅ 4. 가장 많이/적게 한 부위 */
        dto.setMostUsedPart(Optional.ofNullable(exerciseStatusMapper.getMostUsedBodyPart(userId)).orElse("없음"));
        dto.setLeastUsedPart(Optional.ofNullable(exerciseStatusMapper.getLeastUsedBodyPart(userId)).orElse("없음"));
        ...
        dto.setDailyLabels(dailyLabels);
        ...
        dto.setBalanceLabels(balanceLabels);
        dto.setBalanceData(balanceData);

        /** ✅ 7. 퍼스널 레코드 */
        dto.setPersonalRecords(exerciseStatusMapper.getPersonalRecords(userId));
